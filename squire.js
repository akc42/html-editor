...
const isIOS = /iP(?:ad|hone|od)/.test(ua) || isMac && !!navigator.maxTouchPoints;
...
const supportsInputEvents = "onbeforeinput" in document && "inputType" in new InputEvent("input");
...
function resetNodeCategoryCache () {
...
function getNodeBeforeOffset(node, offset) {
...
function getNodeAfterOffset(node, offset) {
  let returnNode = node;
...
function moveRangeBoundaryOutOf(range, tag, root) {
...
function cleanTree(node, config, preserveWS) {
  const children = node.childNodes;
...
function getTextContentsOfRange(range) {
  if (range.collapsed) {
    return "";
  }
  const startContainer = range.startContainer;
...
function extractRangeToClipboard(event, range, root, removeRangeFromDocument, toCleanHTML, toPlainText, plainTextOnly) {
...
function _monitorShiftKey(event) {
...
export default class Squire {
  constructor(root, config) {
    ...
    */
    ...
    this._keyHandlers = {
      ...
      "Delete": this._Delete,
      "Tab": this._Tab,
      "Shift-Tab": this._ShiftTab,
      " ": this._Space,
      "ArrowLeft": this._removeZWS,
      "ArrowRight": (event, range) => {
        this._removeZWS();
        if (rangeDoesEndAtBlockBoundary(range, this._root)) {
          moveRangeBoundariesDownTree(range);
          ...
          } while (!node.nextSibling && (node = node.parentNode) && node !== this._root);
        }
      },
      [ctrlKey + "b"]:  this._mapKeyToFormat("B");
      [ctrlKey + "i"]:  this._mapKeyToFormat("I");
      [ctrlKey + "u"]:  this._mapKeyToFormat("U");
      [ctrlKey + "Shift-7"]:  this._mapKeyToFormat("S");
      [ctrlKey + "Shift-5"]:  this._mapKeyToFormat("SUB", { tag: "SUP" });
      [ctrlKey + "Shift-6"]:  this._mapKeyToFormat("SUP", { tag: "SUB" });
    ...
    if (!supportsInputEvents) {
      this._keyHandlers.Enter = this._Enter;
      this._keyHandlers["Shift-Enter"] = this._Enter;
    }
    if (!isMac && !isIOS) {
      this._keyHandlers.PageUp = () => {
        this.moveCursorToStart();
      };
      this._keyHandlers.PageDown = () => {
  ...
  decreaseListLevel(range) {
    ...
    if (insertBefore) {
      this._mergeContainers(insertBefore);
    }
  ...
  modifyBlocks(modify, range) {
    ...
    if (range.endOffset < range.endContainer.childNodes.length) {
      this._mergeContainers(range.endContainer.childNodes[range.endOffset]);
    }
    this._mergeContainers(range.startContainer.childNodes[range.startOffset]);
    this._getRangeAndRemoveBookmark(range);
    this.setSelection(range);
    this._updatePath(range, true);
    return this;
  ...
  _afterDelete(range) {
    try {
      ...
      if (node !== parent) {
        ...
        parent.removeChild(node);
        if (!isBlock(parent)) {
          parent = getPreviousBlock(parent, this._root) || this._root;
        }
        fixCursor(parent);
        moveRangeBoundariesDownTree(range);
      }
  ...
  _Backspace( event, range) {
    ...
    if (!range.collapsed) {
      ...
      this._fixContainer(current.parentNode);
  ...
  _Delete(event, range) {
    ...
    if (!range.collapsed) {
      ...
      deleteContentsOfRange(range, this._root);
      this._afterDelete(range);
    } else if (rangeDoesEndAtBlockBoundary(range, this._root)) {
      event.preventDefault();
      current = getStartBlockOfRange(range, this._root);
      if (!current) {
        return;
      }
      this._fixContainer(current.parentNode);
      next = getNextBlock(current, this._root);
      if (next) {
        if (!next.isContentEditable) {
          detachUneditableNode(next, this._root);
          return;
        }
        mergeWithBlock(current, next, range, this._root);
        next = current.parentNode;
        while (next !== this._root && !next.nextSibling) {
          next = next.parentNode;
        }
        if (next !== this._root && (next = next.nextSibling)) {
          this._mergeContainers(next);
        }
        this.setSelection(range);
        this._updatePath(range, true);
      ...
      moveRangeBoundariesUpTree(range, this._root, this._root, this._root);
  ...
  _fixContainer(container) {
    ...
    Array.from(container.childNodes).forEach((child) => {
      ...
      if (!isBR && isInline(child)) {
        if (!wrapper) {
          wrapper = createElement(this._config.blockTag);
        }
  ...
  _insertTreeFragmentIntoRange(range, frag) {
    ...
    moveRangeBoundariesDownTree(range);
    range.collapse(false);
    const stopPoint = getNearest(range.endContainer, this._root, "BLOCKQUOTE") || this._root;
    let block = getStartBlockOfRange(range, this._root);
    ...
    !getNearest(firstBlockInFrag, frag, "PRE") && !getNearest(firstBlockInFrag, frag, "TABLE")) {
      moveRangeBoundariesUpTree(range, block, block, this._root);
      range.collapse(true);
      let container = range.endContainer;
      let offset = range.endOffset;
      this._cleanupBRs(block, false);
      if (isInline(container)) {
        const nodeAfterSplit = split(
          container,
          offset,
          getPreviousBlock(container, this._root) || this._root,
          this._root
        );
      ...
      mergeWithBlock(container, firstBlockInFrag, range, this._root);
    ...
    if (getLength(frag)) {
      ...
      moveRangeBoundariesUpTree(range, stopPoint, stopPoint, this._root);
      ...
      block = getEndBlockOfRange(range, this._root);
      moveRangeBoundariesDownTree(range);
      const container = range.endContainer;
    ...
    if (blockContentsAfterSplit && block) {
      ...
      mergeWithBlock(block, blockContentsAfterSplit, tempRange, this._root);
  ...
  _linkifyText(textNode, offset) {
    ...
    const match = this.linkRegExp.exec(searchText);
    if (match) {
  ...
  _mergeContainers(node) {
    ...
    if (prev && areAlike(prev, node)) {
      ...
      if (needsFix) {
        this._fixContainer(prev);
      }
      if (first) {
        this._mergeContainers(first);
      }
    } else if (isListItem) {
      const block = createElement(this.config.blockTag);
  ...
  _ShiftTab( event, range) {
    this._removeZWS();
    if (range.collapsed && rangeDoesStartAtBlockBoundary(range, this._root)) {
      const node = range.startContainer;
      if (getNearest(node, this._root, "UL") || getNearest(node, this._root, "OL")) {
        event.preventDefault();
        this.decreaseListLevel(range);
      }
    }
  ...
  _Space( event, range) {
    ...
    if (!range.collapsed) {
      ...
      const block = getStartBlockOfRange(range, this._root);
  ...
  _splitBlock(lineBreakOnly, range) {
    ...
    if (this._config.addLinks) {
      ...
      setTimeout(() => {
        this._linkifyText(this, textNode, offset2);
      }, 0);
    }
  ...
  _Tab(event, range) {
    this._removeZWS();
    if (range.collapsed && rangeDoesStartAtBlockBoundary(range, this._root)) {
      let node = getStartBlockOfRange(range, this._root);
      let parent;
      while (parent = node.parentNode) {
